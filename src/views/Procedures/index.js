...
export default function Procedures({ navigation, route }) {
      ...
      <Card key={item.id} style={styles.cardContainer}>
        ...
        >
          <Body>
            ...
            >
              Gestión: #{item.id} {item.prospect ? ` - Prospect: #${item.prospect.id}` : null}
            </Text>
  ...
  const renderFooter = () => {
    return (
      !loading ? end &&
        <View><Text style={{ textAlign: 'center' }} id="lblNoMoreProcedures">No hay más gestiones para cargar</Text></View>
        :
      <View >
        <Spinner
          testID="spinner"
